# auto-publish-timestamp Specification

## Purpose

定义「首次发布时间」的自动捕获能力：以「`published=true` 且无 `publishedAt`」为首次发布信号，在部署构建前回写秒级 `publishedAt`，并通过提交信息哨兵与并发控制规避流水线自触发环。设计背景与跨仓库关系见 `knowledge/architecture/deploy-publish-timestamp.md`。
