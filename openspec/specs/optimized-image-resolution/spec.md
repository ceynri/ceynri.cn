# optimized-image-resolution Specification

## Purpose

提供「内容源正文图片 → 发布优化图 URL」的解析能力。复用 Astro content 管线维护的图片元数据映射（`astro:asset-imports` 的 imageAssetMap）与官方 `getImage`，使独立 endpoint（如 Markdown 孪生）能拿到与正文页同源、同 hash、自动落盘的优化图 URL，且不对外暴露原图。仅覆盖正文 image 节点引用的图；link 节点指向的图不在 Astro 图片映射中，不在本能力范围。
